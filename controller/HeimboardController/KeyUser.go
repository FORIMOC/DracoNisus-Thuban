...
func UpdateKeyUserStatus(ctx *gin.Context) {
	...
	if err := DB.Where("user_id = ?", userID).First(&keyUser).Error; util.HandleNotFoundErr(err, ctx) {
...
func AddKeyUser(ctx *gin.Context) {
	...
	if err := DB.Create(&newKeyUser).Error; err != nil {
...
func SetKeyUserInfo(ctx *gin.Context) {
	...
	if err := DB.Save(&keyUser).Error; err != nil {
		...
		common.LoggerFile.Error("daatabase error: " + err.Error())
		return
	}
...
func GetKeyUserList(ctx *gin.Context) {
	...
	if err := DB.Find(&keyUserList).Error; util.HandleNotFoundErr(err, ctx) {
...
func GetKeyUserGroup(ctx *gin.Context) {
	...
	var count int64
	if err := DB.Where("user_id = ?", userID).Order("freq DESC").Limit(groupUserLimit).Find(&keyUserGroups).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此关键用户")
		return
	}
...
func GetKeyUserTime(ctx *gin.Context) {
	...
	if err := DB.Where("user_id = ?", userID).Find(&keyUserTimes).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此关键用户")
		return
	}
...
func GetKeyUserKeywords(ctx *gin.Context) {
	...
	var count int64
	if err := DB.Where("user_id = ?", userID).Order("freq DESC").Limit(50).Find(&keywords).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此关键用户")
		return
	}
...
func GetKeyUserCacheMessages(ctx *gin.Context) {
	...
	if err := DB.Where("user_id = ?", userID).Find(&keyUserCacheMessages).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此关键用户")
		return
	}
