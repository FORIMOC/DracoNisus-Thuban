...
func AddKeyword(ctx *gin.Context) {
	...
	if err := DB.Where("keyword = ?", keyword).First(&keywordList).Error; util.HandleNotFoundErr(err, ctx) {
	...
	if err := DB.Create(&newKeyword).Error; err != nil {
...
func GetKeywordList(ctx *gin.Context) {
	...
	// keywords 数组
	var keywordList []ModelKeyword.KeywordList
	if err := DB.Find(&keywordList).Error; util.HandleNotFoundErr(err, ctx) {
...
func RegulateKeyword(ctx *gin.Context) {
	...
	if err := DB.Find(&keyword).Error; util.HandleNotFoundErr(err, ctx) {
