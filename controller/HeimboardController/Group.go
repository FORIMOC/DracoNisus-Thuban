...
func UpdateGroupStatus(ctx *gin.Context) {
	...
	if err := DB.Where("group_id = ?", groupID).First(&group).Error; util.HandleNotFoundErr(err, ctx) {
...
func GetGroupList(ctx *gin.Context) {
	...
	if err := DB.Where("status != 0").Find(&groups).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusOK, 200, nil, "未找到任何群聊")
		common.LoggerFile.Error("database error: " + err.Error())
		return
	}
...
func GetGroupGraph(ctx *gin.Context) {
	...
	if err := DB.Where("group_id = ?", groupID).Order("value DESC").Limit(groupGraphPointLimit).Find(&users).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此群聊")
		return
	}
	...
	for _, user := range users {
		...
		if err := DB.Where("group_id = ? AND user_id1 = ?", groupID, user.UserID).Find(&tempGraphs).Error; util.HandleNotFoundErr(err, ctx) {
...
func GetUserList(ctx *gin.Context) {
	...
	var users []ModelGroup.GroupUser
	var count int64
	if err := DB.Where("group_id = ?", groupID).Order("freq DESC").Limit(groupUserLimit).Find(&users).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此群聊")
		return
	}
...
func GetGroupCacheMessages(ctx *gin.Context) {
	...
	if err := DB.Where("group_id = ?", groupID).Find(&cacheMessages).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此群聊")
		return
	}
...
func GetGroupKeywords(ctx *gin.Context) {
	...
	var count int64
	if err := DB.Where("group_id = ?", groupID).Order("freq DESC").Limit(limit).Find(&keywords).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此群聊")
		return
	}
...
func GetGroupTimes(ctx *gin.Context) {
	...
	if err := DB.Where("group_id = ?", groupID).Find(&times).Error; util.HandleNotFoundErr(err, ctx) {
		common.Response(ctx, http.StatusUnprocessableEntity, 422, nil, "不存在此群聊")
		return
	}
